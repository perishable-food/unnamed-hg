...
#define NELEMS_POKEFORMDATATBL 294
...
u32 LONG_CALL UseItemMonAttrChangeCheck(struct PLIST_WORK *wk, void *dat)
{
     ...
     && CanUseRevealGlass(pp) == TRUE)
    {
	 ...
	 if (wk->dat->item == ITEM_FIGHTING_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 1)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 1; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_FLYING_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 2)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 2; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_POISON_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 3)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 3; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_GROUND_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 4)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 4; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_ROCK_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 5)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 5; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_BUG_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 6)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 6; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_GHOST_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 7)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 7; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_STEEL_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 8)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 8; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_FAIRY_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 9)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 9; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_FIRE_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 10)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 10; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_WATER_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 11)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 11; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_GRASS_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 12)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 12; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_ELECTRIC_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 13)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 13; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_PSYCHIC_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 14)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 14; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_ICE_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 15)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 15; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_DRAGON_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 16)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 16; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
	...
	if (wk->dat->item == ITEM_DARK_MEMORY
     && CanUseMemories(pp) == TRUE)
    {
        if (GetMonData(pp, MON_DATA_FORM, NULL) == 17)
            wk->dat->after_mons = 0; // change to normal forme
        else
            wk->dat->after_mons = 17; // change to memory type
        sys_FreeMemoryEz(dat);
        PokeList_FormDemoOverlayLoad(wk);
        ChangePartyPokemonToForm(pp, wk->dat->after_mons); // this works alright
        return TRUE;
...
const u8 sTrainerGenders[] = {
	...
	[TRAINERCLASS_MARY] = TRAINER_FEMALE,
};
