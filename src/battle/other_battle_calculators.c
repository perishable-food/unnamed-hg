...
BOOL CantEscape(void *bw, struct BattleStruct *sp, int battlerId, MESSAGE_PARAM *msg) {
    ...
    if (item == HOLD_EFFECT_FLEE || (battleType & BATTLE_TYPE_NO_EXPERIENCE) /*|| GetBattlerAbility(ctx, battlerId) == ABILITY_RUN_AWAY*/ || BATTLE_MON_HAS_TYPE(sp, battlerId, TYPE_GHOST)) {
        return FALSE;
    }
...
BOOL BattleTryRun(void *bw, struct BattleStruct *sp, int battlerId) {
    ...
    if (item == HOLD_EFFECT_FLEE) {
        ...
        ret = TRUE;
    } else if (GetBattlerAbility(sp, battlerId) == ABILITY_WANDERING_SPIRIT) {
